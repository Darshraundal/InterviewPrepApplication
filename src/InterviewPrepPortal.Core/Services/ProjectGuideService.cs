...
/// </summary>
public class ProjectGuideService(IJsonDataLocator dataLocator, IMemoryCache cache, ILogger<ProjectGuideService> logger)
    : IProjectGuideService
{
    ...
    {
        var path = Path.Combine(dataLocator.JsonDataRoot, "projects", "projects-index.json");
        if (!File.Exists(path))
        {
    ...
    private async Task<Dictionary<string, ProjectDetail>> LoadProjectDetailsAsync(IReadOnlyList<ProjectGuide> index)
    {
        ...
        foreach (var project in index)
        {
            ...
            var filePath = Path.Combine(dataLocator.JsonDataRoot, "projects", project.DataFile);
