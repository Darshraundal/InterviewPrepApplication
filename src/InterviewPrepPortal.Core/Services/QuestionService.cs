...
/// </summary>
public class QuestionService(IJsonDataLocator dataLocator, IMemoryCache cache, ILogger<QuestionService> logger)
    : IQuestionService
{
    ...
    {
        var path = Path.Combine(dataLocator.JsonDataRoot, "categories.json");
        if (!File.Exists(path))
        {
    ...
    {
        var jsonDataPath = dataLocator.JsonDataRoot;
